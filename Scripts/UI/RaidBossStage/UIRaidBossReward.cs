...
public class UIRaidBossReward : UIDataItem<RaidBossReward>
{
    ...
    private void SetupInfo(RaidBossReward data)
    {
        if (textRank != null)
        {
            if (data.rankMin == data.rankMax)
                textRank.text = data.rankMin.ToString("N0");
            else
                textRank.text = data.rankMin + "-" + data.rankMax;
        }
